"""
The contents of this file were taken as is from https://github.com/hill-a/stable-baselines

The requirements for both open/baselines and hill-a/stable-baselines are getting harder and harder to satisfy, so copying it was just easier.

tensorflow<=2.0.0? gym<0.16.0?! Really?!

Anyway, their license is MIT, if mine is not compatible I am more than willing to change it.
"""
import numpy as np
...
class SumSegmentTree(SegmentTree):
   ...
   def find_prefixsum_idx(self, prefixsum):
      ...
      while np.any(cont):  # while not all nodes are leafs
         idx[cont] = 2 * idx[cont]
         prefixsum_new = np.where(self._value[idx] <= prefixsum,
