...
class DRLController(BaseController, ABC):
   ...
   def __init__(self,
      ...
      self.graph_layer_type = HeteroMAGNet.GraphLayerType[
          graph_layer_type] if graph_layer_type is not None else None
      ...
      self.policy_net = HeteroMAGNet(self.action_layer_type,
                                     share_encoding,
                                     share_comms,
                                     share_action,
                                     self.num_unit_types,
   ...
   @staticmethod
   def _get_trr(network: HeteroMAGNet,
                states: list,
                next_states: list,
      ...
      final_dist1 = th.zeros(
          (valid_states.num_nodes, valid_states.num_nodes, network.relational_layer.n_heads),
          device=network.device)
      final_dist2 = th.zeros((non_final_next_states_batch.num_nodes,
...
class DQNController(DRLController):
   ...
   def __init__(self,
      ...
      self.target_net = HeteroMAGNet(self.action_layer_type,
                                     share_encoding,
                                     share_comms,
                                     share_action,
                                     self.num_unit_types,
   ...
   def normalize_batch_length(eps, device):
      ...
      dummy_state = Data(x=dummy_x, edge_index=dummy_edge_index)
      ...
      for i, episode in enumerate(eps):
         ...
         states_in_device = [s.to(device)
                             for s in batch.state] + [dummy_state] * (max_size - len(batch.state))

         action = th.stack(batch.action).long()
         action = th.cat(
             (action, th.zeros((max_size - action.size(0), action.size(1)), dtype=th.long)))
      ...
      return state_batch, action_batch, reward_batch, done_batch, av_actions_batch, dummy

   def optimize(self, step_num: int, trainer: TrainingConfig, memory):
      assert memory.can_sample(trainer.batch_size), "Replay memory hasn't reached the minimum size for an optimization step"
      ...
      state_batch, action_batch, reward_batch, done_batch, av_actions_batch, dummy = self.normalize_batch_length(
          episodes, self.device)
      ...
      current_q_vals = th.empty(len(state_batch),
                                len(state_batch[0]),
                                self.n_agents,
                                self.n_actions_agents[0],
                                device=self.device)
      target_q_vals = th.empty(len(state_batch),
                               len(state_batch[0]),
                               self.n_agents,
                               self.n_actions_agents[0],
                               device=self.device)
      ...
      for t in range(len(state_batch[0])):
         ...
         graph_state_batch = Batch.from_data_list(time_step)
      ...
      if memory.is_prioritized:
         ...
         memory.update_priorities(batch_idxes, priorities.tolist())
      ...
      loggers_poggers = {'losses/loss': float(total_loss)}
      if step_num - self.last_grad_log > trainer.max_num_steps / 2000:
         self.last_grad_log = step_num
         for tag, value in self.policy_net.named_parameters():
            tag = tag.replace('.', '/')
