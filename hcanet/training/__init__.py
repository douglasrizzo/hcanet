...
from ..rl.replay import (EpisodeReplayBuffer,
...
class TrainingConfig:
   ...
   def initialize(self):
      ...
      parser.add_argument('--checkpoint_save_secs',
                          default=180,
                          type=int,
      ...
      parser.add_argument(
          ...
          default='smart',
          type=str,
          help='torch device ("cpu", "cuda", "cuda:0" etc.). "smart" selects the CPU if no GPU available, or the only GPU, or the GPU with most memory available, or errors'
      )
      ...
      parser.add_argument('--eps_anneal_time',
                          default=50000,
                          type=float,
      ...
      parser.add_argument('--full_agent_communication',
      ...
      parser.add_argument('--episode_priority',
                          default=None,
                          type=str,
      ...
      parser.add_argument('--max_trajectory_length',
                          default=None,
                          type=int,
      ...
      parser.add_argument('--replay_buffer_alpha',
                          default=.6,
                          type=float,
                          help='value for the alpha param of the prioritized replay buffer')
      parser.add_argument('--replay_buffer_beta',
                          default=.4,
                          type=float,
      ...
      parser.add_argument('--rgcn_num_bases',
                          default=1,
                          type=int,
                          help='number of basis matrices for RGCN layers')
      parser.add_argument('--rgcn_fast',
                          dest='rgcn_fast',
                          action='store_true',
                          default=False,
                          help='use FastRGCN instead of RGCN')
      parser.add_argument('--rmsprop_alpha', default=.99, type=float, help='RMSprop alpha')
      parser.add_argument('--rmsprop_eps', default=0.00001, type=float, help='RMSprop eps')
      parser.add_argument('--sparse_rewards',
                          dest='sparse_rewards',
                          action='store_true',
                          default=False,
      ...
      parser.add_argument(
          ...
          action='store_true',
          default=False,
          help='save replays from evaluation episodes, ((max_steps / eval_interval) * eval_episodes) SC2Replay files will be created'
      )
      parser.add_argument(
          '--share_encoding',
          dest='share_encoding',
      ...
      parser.add_argument(
          ...
          action='store_true',
          default=False,
          help='share the same parameters in the communication module for all agent classes (in the case of RGCN only)'
      )
      parser.add_argument('--share_action',
                          dest='share_action',
                          action='store_true',
                          default=False,
                          help='share the same network in the action module for all agent classes')
      parser.add_argument('--trr_coef',
                          default=0,
                          type=float,
      ...
      parser.add_argument('--rgcn_n1_relations',
      ...
      if dev == 'smart':
         ...
         if len(gpus) == 0:
            ...
            available_gpus = GPUtil.getAvailable(order='memory', memoryFree=1500, maxLoad=1)
   ...
   def get_loggable_args(self):
      ...
      if self.action_module != TrainingConfig.ActionModuleType.RANDOM:
         d['NETWORK'] = [
             ...
             ('full_receptive_field', 'fullrcp' if self.full_receptive_field else 'nofullrcp'),
         ...
         if self.action_module in [
             TrainingConfig.ActionModuleType.DQN, TrainingConfig.ActionModuleType.DDQN]:
            d['OFF-POLICY'] = [
